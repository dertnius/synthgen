...
.EXAMPLE
scripts\setup-enterprise.ps1 -Mode mirror -NuGetMirrorUrl https://artifacts.corp.example/api/nuget/v3/index.json -NonInteractive
#>
...
function Get-NuGetConfigContent {
    ...
    $xml.Add('<!-- Generated by scripts/setup-enterprise.ps1 from enterprise-profile(.local).psd1.')
    ...
    if ($SkipVerify) {
        ...
        if ($LASTEXITCODE -ne 0) {
            if ($cfg.Mode -eq 'mirror') {
                ...
                Fail 6 'VERIFY:RESTORE' "restore from the '$($cfg.NuGet.OfflineFeedPath)' folder feed failed. The feed may be incomplete - regenerate it on a connected machine with scripts/export-offline-feed.ps1 and re-copy. NU1603 'approximate best match' warnings are expected and benign."
            }
        }
